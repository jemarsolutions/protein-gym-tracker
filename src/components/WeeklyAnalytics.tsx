...
export default function WeeklyAnalytics({ isPremium, proteinByDay, gymDays, goal }: Props) {
  ...
  return (
    <div className="relative w-full max-w-md mx-auto rounded-[2rem] overflow-hidden shadow-lg dark:shadow-2xl">
      {/* Card */}
      <div className={`p-6 bg-white dark:bg-slate-900/60 backdrop-blur-2xl border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl transition-all duration-500 ${!isPremium ? "select-none" : ""}`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3 text-sm font-semibold">
            ...
            <span className="text-indigo-600 dark:text-indigo-400">Avg {avgProtein}g</span>
          </div>
        </div>
                ...
                <div className="w-full flex flex-col justify-end" style={{ height: "100%" }}>
                  <div
                    className={`w-full rounded-xl transition-all duration-700 ease-out ${
                      hitGoal
                        ? "bg-gradient-to-t from-emerald-600 to-emerald-450 dark:from-emerald-600 dark:to-emerald-400"
                        : "bg-gradient-to-t from-indigo-600 to-indigo-450 dark:from-indigo-700 dark:to-indigo-500"
                    }`}
                    style={{ height: `${heightPct}%` }}
                  />
                </div>
                {/* Gym dot */}
                <div className={`w-2 h-2 rounded-full mt-1 ${isGym ? "bg-amber-500 dark:bg-amber-400" : "bg-slate-200 dark:bg-slate-700"}`} />
                <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">{getDayLabel(date)}</span>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400 font-medium">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-emerald-450 dark:bg-emerald-400 inline-block" /> Goal hit
          </span>
          <span className="flex items-center gap-1.5">
      ...
      {!isPremium && (
        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-[2rem] backdrop-blur-[4px] bg-white/40 dark:bg-slate-950/60 border border-slate-200/50 dark:border-indigo-500/20">
          <div className="flex flex-col items-center text-center px-6">
            ...
            <h3 className="text-slate-950 dark:text-white font-bold text-lg mb-1">Weekly Analytics</h3>
